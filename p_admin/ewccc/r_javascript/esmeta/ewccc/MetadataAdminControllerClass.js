...
function MetadataAdminControllerClass(){
	this.error = false;
};
...
MetadataAdminControllerClass.prototype.writeFooter = function(){
  	var FECHA = new Date();
...
MetadataAdminControllerClass.prototype.readEsmetaXML = function(tag,data){
	var openTag = "<" + tag + ">";
	var closedTag = "</" + tag + ">";
...
MetadataAdminControllerClass.prototype.getPageSize = function(){
...
MetadataAdminControllerClass.prototype.trimString = function(str){
...
MetadataAdminControllerClass.prototype.showWebForm = function(){
 		var arrayPageSize = this.getPageSize();
 		
 		var objOverlay = document.getElementById("overlay");
		objOverlay.style.display = "block";
  		var loginContainer = document.getElementById("METADATA1");
...
MetadataAdminControllerClass.prototype.uploadMetadata = function(){
		mObject = new MetadataAdminControllerClass();
		mObject.getMetadataInfo();


		this.showWebForm();
		
		document.forms["METADATA1"].reset();
		document.forms["METADATA1"].TITLE.focus();
}

MetadataAdminControllerClass.prototype.closeWebForm = function(){
	var obj = document.getElementById("METADATA1");
...
MetadataAdminControllerClass.prototype.getMetadataInfo = function(){
	var url = "r_php/ewccc/metadata/getMetadataInfo.php";
	var pars = '';
	...
	method: 'post',
	parameters: pars,
	onComplete: this.showResponseGetMetadataInfo,
	onFailure: this.reportError
	});
}

MetadataAdminControllerClass.prototype.showResponseGetMetadataInfo = function (originalRequest){
	...
	if(message != -1){
		...
		if(message != -1){
			var FORM_OBJECT = document.forms["METADATA1"];
			FORM_OBJECT.TITLE.value = this.trimString(readEsmetaXML("title",data));
			FORM_OBJECT.DESCRIPTION.value = this.trimString(readEsmetaXML("description",data));
			FORM_OBJECT.KEYWORDS.value = this.trimString(readEsmetaXML("keywords",data));
...
MetadataAdminControllerClass.prototype.onSubmitCreateAccount = function(){
	return false;
}

MetadataAdminControllerClass.prototype.onClickUpdateMetadata = function(){
	var TITLE = escape(this.trimString(document.forms["METADATA1"].TITLE.value));
	var DESCRIPTION = escape(this.trimString(document.forms["METADATA1"].DESCRIPTION.value));
	var KEYWORDS = escape(this.trimString(document.forms["METADATA1"].KEYWORDS.value));

	if(TITLE == ''){
		alert("Please, Write the  metadata's title");
		document.forms["METADATA1"].TITLE.focus();
		return false;
	}else if(DESCRIPTION == ''){
		alert("Please, Write the metadata's description");
		document.forms["METADATA1"].DESCRIPTION.focus();
		return false;
	}else if(KEYWORDS == ''){
		alert("Please, Write the metadata's keywords");
		document.forms["METADATA1"].KEYWORDS.focus();
		return false;
	}else{
		var OBJ;
		OBJ = document.getElementById("INDICATOR_UPDATE_METADATA");
		OBJ.style.display = "block";
	
		this.AJAX_updateMetadata(TITLE,DESCRIPTION,KEYWORDS);
	}
}

MetadataAdminControllerClass.prototype.AJAX_updateMetadata = function(TITLE,DESCRIPTION,KEYWORDS){

var url = "r_php/ewccc/metadata/updateMetadata.php";
var pars = 'TITLE='+ TITLE;
pars +='&DESCRIPTION=' + DESCRIPTION + '&KEYWORDS=' + KEYWORDS;
...
method: 'post',
parameters: pars,
onComplete: this.showResponseUpdateMetadata,
onFailure: this.reportError
});

}

MetadataAdminControllerClass.prototype.showResponseUpdateMetadata = function (originalRequest){
	...
	if(message != -1){
		message = readEsmetaXML("transaction",data);
		if(message != -1){
			alert('Thanks, Metadata has been updated.');
			
			mObject = new MetadataAdminControllerClass();
			mObject.closeWebForm();
...
MetadataAdminControllerClass.prototype.updateDocumentInfo = function(IDDOCUMENT){
...
MetadataAdminControllerClass.prototype.setAJAX_Indicator = function (IDPREFIX,ID,onoff){
		var IDDIV = ID;
...
MetadataAdminControllerClass.prototype.setDIV_HS = function (IDDIV,onoff){
...
MetadataAdminControllerClass.prototype.setDIV_HTML = function (IDDIV,HTML){
		OBJ = document.getElementById(IDDIV);
		OBJ.innerHTML = HTML;
}


MetadataAdminControllerClass.prototype.showResponseGetDocumentInfo = function (originalRequest){
...
MetadataAdminControllerClass.prototype.onClickUpdateDocument = function(){
	var IDDOCUMENT = this.trimString(document.forms["DOCUMENT"].IDDOCUMENT.value);
	var TITLE = this.trimString(document.forms["DOCUMENT"].TITLE.value);
	var CONTENT = this.trimString(document.forms["DOCUMENT"].CONTENT.value);
	var IDPARENT = this.trimString(document.forms["DOCUMENT"].IDPARENT.value);
	var DESCRIPTION = this.trimString(document.forms["DOCUMENT"].DESCRIPTION.value);
...
MetadataAdminControllerClass.prototype.AJAX_UpdateDocument = function(IDDOCUMENT,TITLE,CONTENT,IDPARENT,DESCRIPTION,KEYWORDS){
...
MetadataAdminControllerClass.prototype.showResponseUpdateDocumentInfo = function (originalRequest){
...
MetadataAdminControllerClass.prototype.deleteDocument = function(IDDOCUMENT){
...
MetadataAdminControllerClass.prototype.showResponseDeleteDocument = function (originalRequest){
...
MetadataAdminControllerClass.prototype.window_onLoad = function(){
	//Empty
};