...
function writeMetadata($IDDOCUMENT){
	...
	echo("		<!-- KEYWORDS -->\n");
	echo("		<meta name=\"keywords\" content=\"" . $row["METADATA_KEYWORDS"] . "\" />\n");
	echo("		<meta http-equiv=\"keywords\" content=\"" . $row["METADATA_KEYWORDS"] . "\" />\n\n");
	
	echo("		<!-- DESCRIPCION -->\n");
	echo("		<meta name=\"description\" content=\"" . $row["METADATA_DESCRIPTION"] . "\" />\n");
	echo("		<meta http-equiv=\"description\" content=\"" . $row["METADATA_DESCRIPTION"] . "\" />\n");
	echo("		<meta http-equiv=\"DC.Description\" content=\"" . $row["METADATA_DESCRIPTION"] . "\" />\n\n");
	...
	echo("		<meta name='DC.Language' SCHEME='RFC1766' CONTENT='English' />\n");
	echo("		<meta name='distribution' content='global' />\n");
	echo("		<meta name='resource-type' content='document' />\n\n");

	echo("		<meta name='Author' content='Juan Antonio Breña Moral' />\n");
	...
	echo("		<!-- RSS -->\n");
	echo("		<link rel='alternate' type='application/rss+xml' title='Juan Antonio Breña Moral RSS' href='http://feeds.feedburner.com/esmetaman/' />\n\n");
	
	//echo("		<meta name='verify-v1' content='WIaS7oITIXqokH+Rg8hDQcTHJyriPqxFgOd9OAgT1lk=' />\n");	
};

function writeMetadataCMS($IDDOCUMENT){
	...
	$PRE_MESSAGE = "Juan Antonio Breña Moral:";
	$TITLE = getLevel2($IDDOCUMENT);
	$POST_MESSAGE = "";//$row["METADATA_TITLE"];

	echo("\n		<!-- ESMETA METADATA SCRIPT 1.0 -->\n");
	echo("		<!-- TITLE -->\n");
	echo("		<title>" . $PRE_MESSAGE . " " . $TITLE . " " . $POST_MESSAGE .  "</title>\n");
	echo("		<meta name=\"title\" content=\"" . $PRE_MESSAGE . " " . $TITLE . " " . $POST_MESSAGE .  "\" />\n");
	echo("		<meta name=\"DC.Title\" content=\"" . $PRE_MESSAGE . " " . $TITLE . " " . $POST_MESSAGE .  "\" />\n");
	echo("		<meta http-equiv=\"title\" content=\"" . $PRE_MESSAGE . " " . $TITLE . " " . $POST_MESSAGE .  "\" />\n\n");
...
function getCount($IDDOCUMENT){
	
	mysql_connect(DB_HOST, DB_USER, DB_PASS) or die(mysql_error());
	mysql_select_db(DB_NAME) or die(mysql_error());
	
	$sql = "SELECT COUNTER FROM ewccc_documents WHERE IDDOCUMENT=" . $IDDOCUMENT;
	//echo $sql;
	$result = mysql_query($sql) or die(mysql_error());
	$row = mysql_fetch_array($result);
...
function updateCounter($IDDOCUMENT){
	$CURRENT_COUNTER = getCount($IDDOCUMENT);
	$CURRENT_COUNTER = $CURRENT_COUNTER + 1;
	
	$sql = "";
	$sql .= "UPDATE ewccc_documents SET COUNTER = " . $CURRENT_COUNTER;	
